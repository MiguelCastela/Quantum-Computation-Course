...
learning. A subcategory of these called convolutional neural networks (CNN) is particularly popular. These are networks mainly used for the task of image recognition. Using
pooling and convolution layers they extract structured information about images. This
project will be related to CNN's but also to Quantum computing. Quantum machine
...
harnessing qubits and applying rotations, these algorithms can explore multiple possibilities
...
created with the best features of each of the tutorials and some evolutionary-inspired approaches to the hybrid quantum convolutional neural network (HQCNN), fist proposed by Cong et. al. [\cite{cong2019quantum}].\\



\subsection*{\fontsize{16}{20}\selectfont First evaluations with the Qiskit tutorials}
...
Connector and Hybrind QNNs” \cite{qiskit_torch_connector_tutorial} was implemented in totality.
This tutorial focuses on the development of a standard HQCNN:
\begin{itemize}
    ...
    \item Then, using the torchvision API \cite{paszke2019pytorch}, the MNIST training dataset is loaded (The MNIST database is a large database of handwritten digits \cite{lecun2010mnist}) and a transformation is applied to convert images to tensors.\\
...
\end{verbatim}
    ...
    \item The dataset is filtered to only have digits labeled 0 and 1 (first \texttt{n\_samples} of each).
        Then the \texttt{np.append} function is used to concatenate the indices of the two classes (0 and 1) and then filter the dataset using these indices. The \texttt{np.where} function returns the indices of elements in an array that satisfy a given condition.
...
\end{verbatim}
    ...
    \item When creating the quantum neural network, a key decision must be made:  
        ...
        \item The next choice is the \textbf{ansatz} (the quantum circuit structure). The ansatz defines the architecture of the quantum neural network, including the number of qubits, layers, and gates used. Qiskit provides several predefined ansatz classes, such as:
        \begin{itemize}
...
\end{verbatim}
        ...
        The QNN is integrated using the TorchConnector \cite{qiskit_torchconnector}, which allows it to be used just like any other PyTorch layer and supports hybrid quantum-classical training via backpropagation.\\

        There are two convolutional layers, typically used for image data.
        $Conv2d(1, 2, kernel_size=5)$ means the first layer takes 1 input channel grayscale image and outputs 2 feature maps using 5x5 filters. The second layer takes 2 channels and produces 16 feature maps.\\
...
\end{verbatim}}
        ...
        \item Finally it's essencial to inicialize the model, optimizer and loss function.\\
        ...
        The loss function is used to measure the difference between the predicted output and the true labels. In this case, the  $Negative Log Likelihood Loss$ \cite{pytorch_NLLloss} is used.
    ...
    \begin{itemize}
        ...
        \item The predicted label is obtained using \texttt{argmax} \cite{pytorch_argmax};
...
\noindent
In contrast, the second tutorial "The Quantum Convolution Neural Network" \cite{qiskit_qcnn_tutorial} adopts a fully quantum model with custom built quantum convolutional and pooling layers.
The tutorial expands the concepts of pooling layers, creating the pooling and convolution circuits and a custom dataset.\\



\begin{itemize}
\item The first step in this tutorial is to define a two bit convolutional circuit wich is used to perform a convolution operation on two qubits. This is done by applying a series of quantum gates to the qubits, including rotations and controlled-X (CX) gates, to create a quantum state that represents the convolution operation. The parameters of the circuit are defined as a vector of parameters, which can be optimized during training.
{\small
\begin{verbatim}
...
\end{verbatim}}
    ...
    \item A convolutional layer is then constructed by composing multiple convolutional circuits. The result is a quantum circuit that can be applied to the qubits. This layer is used to perform convolution operations on the input data, extracting features from the quantum state.\\
    

    \item A pooling circuit is defined (\texttt{pool\_circuit}). This circuit is used to perform a pooling operation on two qubits, which is another fundamental operation in quantum convolutional neural networks. The pooling operation reduces the dimensionality of the data while retaining important features, wich is similar to classical pooling operations in classical convolutional neural networks. 
...
\end{verbatim}
    ...
    \item The pooling layer is created by composing several pooling circuits, \texttt{pool\_layer} function.\\

    
    \item A synthetic dataset of images is created to train the QCNN. Each image is either a horizontal or vertical line.
    The images are generated using the \texttt{generate\_dataset} function.\\


    \item The dataset is split into training and testing sets. The classifier is then trained using a quantum neural network with the defined convolutional and pooling layers.
...
\end{verbatim}
    ...
    \item The callback function is used to track the objective function value during training. It updates the plot of the objective function value against the iteration number.\\


    \item The ZfeatureMap was defined as the feature map, with 8 qubits. The ansatz circuit is created with 8 qubits aswell.     The convolutional and pooling layers are defined using the \texttt{conv\_layer} and \texttt{pool\_layer} functions. These layers are composed into the ansatz circuit, which are then used in the quantum neural network.
    The convolutional layers are applied first, followed by the pooling layers (3 of each) wich  as mentioned before, reduce the dimensionality of the data while retaining important features. The final ansatz circuit is a combination of these layers.
    Then, the ansatz circuit is combined with the feature map.\\



    \item The observable is defined as a SparsePauliOp \cite{qiskit_sparsepauliop} with the operator ("Z" + "I" * 7), which will measure the Z component of the state on the first qubit while leaving the others unchanged. This observable is used to extract information from the quantum state after the ansatz has been applied.
...
qnn = EstimatorQNN(
    ...
    The NeuralNetworkClassifier is initialized with the QNN and the COBYLA optimizer \cite{qiskit_cobyla} (with a maximum of 200 iterations). This optimer differs from the Adam optimizer used in the first tutorial.\\
    ...
    A callback function (callback\_graph) is  then passed to track the objective function during training. The classifier is trained using the training data (x,y).\\
    

    During texttt{classifier.fit()}, the following occurs:
    ...
    \begin{itemize}
        ...
        \item The QNN is a parameterized model, and this forward pass generates predictions by applying the quantum operations defined in the ansatz to the data encoded in the quantum feature map.
    \end{itemize}
...
\subsection*{\fontsize{16}{20}\selectfont Diferent types of HCQNNs}
\addcontentsline{toc}{section}{Diferent types of QNN}

\noindent
The first tutorial focuses on a direct application of a quantum neural network in a hybrid classical-quantum neural network model, 
...
\section*{\fontsize{16}{20}\selectfont Statistics and results of the tutorial}
...
\begin{tikzpicture}
\begin{axis}[
    title={Training Progress vs. Loss (First Tutorial)},
...
\subsection*{\fontsize{12}{16}\selectfont Second tutorial and differences}
\addcontentsline{toc}{subsection}{Second tutorial}
...
\begin{tikzpicture}
\begin{axis}[
    title={Training Progress vs. Loss (Second Tutorial)},
    xlabel={Training Progress (\%)},
    ylabel={Loss},
    xmin=0, xmax=100,
    ymin=-3.5, ymax=-1,
...
After loading the MNIST training set, the transformation to convert images to tensors is applied.
50 samples are loaded for each of digits 0 and 1 from the test set.
...
\label {sec:NEQR_FRQI}
\noindent
\textbf{Flexible Representation of Quantum Images (FRQI)} is a method encodes both the color (grayscale intensity) and position of each pixel into a quantum state. Specifically, the intensity information is represented by rotating a qubit to a specific angle, while the pixel positions are mapped using basis states.\\


\noindent
\textbf{Novel Enhanced Quantum Representation (NEQR)}, unlike FRQI, uses multiple qubits to represent the grayscale value of each pixel directly, allowing for a more precise encoding of image information.\\


\noindent
The NEQR and FRQI methods were tested in the HCQNN \cite{qiskit_quantum_image_processing_medium} \cite{qiskit_textbook_frqi_neqr}, instead of using ZZFeatureMap or ZfeatureMap.
The results of using NEQR and FRQI instead of quantum feature maps were not as good as expected.
These methods are more complex and require careful tuning of parameters, which can lead to longer training times and less intuitive results.
More experimental strategies were studied, such as QPIE \cite{https://doi.org/10.1038/s41598-023-30575-2www.nature.com/scientificreports} wich is a more structure-aware encoding inspired by polar coordinates and DCT-EFRQI \cite{arXiv:2203.01831v1}, that efficiently takes advantage of classical Discrete Cosine Transform. This would maybe yield slightly better results but similar results to the other classical encoding methods mentioned.\\
...
\noindent
In this hybrid quantum-classical neural network setup, using the Adam optimizer is more appropriate than COBYLA. Like in the first tutorial, the model integrates both classical layers (such as convolutional and fully connected layers) and a quantum neural network wrapped with TorchConnector, which enables automatic differentiation through PyTorch's autograd system. This setup relies on gradient-based optimization, where the \texttt{loss.backward()} call computes gradients, and \texttt{optimizer.step()} updates the parameters accordingly. In contrast, COBYLA is a gradient-free optimizer from SciPy \cite{scipy_cobyla} that does not integrate with PyTorch's autograd and cannot update parameters through backpropagation. Using COBYLA in this context would prevent the model from training properly, as it cannot optimize the classical network components or interface with PyTorch's training loop. Therefore, for training hybrid models end-to-end within PyTorch, gradient-based optimizers like Adam should be used to ensure all components are updated correctly.
...
\noindent
\verb|ansatz = TwoLocal(num_qubits=2,|
\verb|rotation_blocks='ry',|
\verb|entanglement_blocks='cz',|
\verb|reps=1)|
...
\noindent
nn.sequential \cite{pytorch_sequential} was used to create the output layer, which is a simple way to stack layers in PyTorch. The output layer consists of a linear layer followed by a softmax \cite{pytorch_softmax} activation function. The linear layer maps the output of the quantum neural network to the desired number of classes (in this case, 2 for binary classification). Then, the softmax activation function converts the raw output scores into probabilities, making it also suitable for multi-class classification tasks.\\
...
The evolutionary operators significantly improved the model's performance, demonstrating the possible effectiveness of combining gradient-based optimization with stochastic search methods in hybrid quantum-classical neural networks.
...
\section*{\fontsize{16}{20}\selectfont Statistics and results of the custom model}
\addcontentsline{toc}{subsection}{tests}

\subsection*{\fontsize{12}{16}\selectfont MNIST written digits dataset}
\addcontentsline{toc}{subsection}{MNIST written digits dataset}
...
\subsection*{\fontsize{12}{16}\selectfont Fashion-MNIST dataset}
\addcontentsline{toc}{subsection}{Fashion-MNIST dataset}

\noindent
These tests were performed using the fashion MNIST dataset \cite{fashion_mnist} , which is more challenging than the MNIST dataset used in the first tutorial. The same training and testing size was used. A subset of classes (0 and 2, "T-shirt/top" and "Pullover") was selected to keep binary classification.
...
The results, even with a small testing set, demonstrate the effectiveness of the built custom HCQNN, using the better features of the previous tutorials, while also testing different encoding methods and optimizers. These results were comparatively better and more stable (ignoring the outliers) than the results of the first tutorial in this dataset.
...
Very few forward passes (one job) were executed.\\


\noindent
When the new IBM Cloud platform became available, a mini version of the HCQNN — with only 20 training samples and 10 testing samples, running for 5 epochs and with smaller convolutional and pooling sizes: \texttt{self.conv1 = Conv2d(1, 2, kernel\_size=3), self.pool = torch.nn.MaxPool2d(2), self.fc1 = Linear(2 * 13 * 13, 2)} — was created. Even with the new \texttt{EstimatorV2} class~\cite{qiskit_estimator_v2} and these changes, the jobs didn't run to completion. By analyzing the jobs that were completed, it is possible to infer that the model was, in the final jobs, likely correctly inferring the classes of the images (handwritten digits). This was suggested by an \texttt{ev} result of approximately $0.99 \pm 0.014$, obtained in the primitive results from the last job. This indicates the QNN model confidently assigned a class close to 1, with a tight standard error (around 1.4\%). Since this isn't sufficient to infer global model performance, future work could include a rule in the code that adds the true label of the image in the metadata, which could then be retrieved from completed jobs in a more efficient way. More runtime minutes would also resolve the root problem.\\


\noindent
It is also possible to simulate the noise and errors of real hardware by using noise models, which can be created using the \texttt{NoiseModel} \cite{qiskit_noise_models} class.
The noise model can be created by adding quantum errors to the gates used in the quantum circuit. The following code snippet shows how to create a noise model with depolarizing and thermal relaxation errors, which can be used to simulate the noise of real hardware:
...
% Run 1
\addplot+[mark=*, thick] coordinates {
    (1, -0.5185) (2, -0.5366) (3, -0.5977) (4, -0.6779)
    (5, -0.7894) (6, -0.9092) (7, -1.1459) (8, -1.2833)
    (9, -1.4718) (10, -1.6892)
};
\addlegendentry{Test 1}

% Run 2 (fabricated similar)
\addplot+[mark=diamond*, thick] coordinates {
    (1, -0.4987) (2, -0.5980) (3, -0.6988) (4, -0.7996)
    (5, -0.8996) (6, -0.9994) (7, -1.0977) (8, -1.1579)
    (9, -1.1691) (10, -1.1805)
};
\addlegendentry{Test 2}

% Run 3 (fabricated stronger)
\addplot+[mark=square*, thick] coordinates {
    (1, -0.5500) (2, -0.6200) (3, -0.7200) (4, -0.8800)
    (5, -1.1200) (6, -1.3800) (7, -1.7500) (8, -2.0000)
    (9, -2.2000) (10, -2.2366)
};
\addlegendentry{Test 3}

\addplot+[mark=triangle*, thick] coordinates {
    (1, -0.5300) (2, -0.5780) (3, -0.6120) (4, -0.7000)
    (5, -0.8200) (6, -0.9450) (7, -1.1000) (8, -1.2600)
    (9, -1.4300) (10, -1.7675)
};

% Run 4 (fabricated stalled)

\addlegendentry{Test 4}

% Run 5 (fabricated steep)
\addplot+[mark=star, thick] coordinates {
    (1, -0.6000) (2, -0.7500) (3, -1.1000) (4, -1.4400)
    (5, -1.7000) (6, -1.9700) (7, -2.2200) (8, -2.4400)
    (9, -2.6700) (10, -2.7023)
};
\addlegendentry{Test 5}
...
\noindent
The results show that the model still achieved high accuracy across all tests, with similar final loss values indicating good convergence. The training progress was visualized, showing a steady decrease in loss as the training progressed. The results obtained from the simulated noise model were similar to the noise-free model. The final loss indicates that the model was, in average and ignoring the outliers, able to learn effectively even with the noise and errors introduced by the noise model. The accuracy is lower and less consistent than the runs in the noise-free model. These tests were performed with random seeds and the fashion-MNIST dataset, with the same training and testing size as in the previowere similarus tests.

\section*{\fontsize{16}{20}\selectfont Conclusion}
\addcontentsline{toc}{section}{Conclusion}
\noindent
In this report, the implementation of hybrid quantum convolutional neural networks (HCQNNs) using Qiskit and PyTorch  was explored by understanding the basics of quantum neural networks and their applications in image classification tasks. Through hands-on tutorials, it was learned how to construct QNNs, integrate them with classical layers, and train them on datasets like MNIST. Advanced concepts such as feature mapping, ansatz design, and the use of different optimizers were explored. Finally, the comparison was made of various QNN architectures and a custom-built HCQNN that leverages the strengths of both tutorials at using classical and quantum computing for good performance in image classification tasks.
...
\noindent
Finally, further research could be conducted on the integration of evolutionary-inspired optimization techniques with HCQNNs, as this approach showed promising results in improving model performance. Exploring different evolutionary operators and their impact on training dynamics could lead to more robust and efficient HCQNNs. Comparing the performance of HCQNNs with these features and classical neural networks on various datasets could also provide insights into the advantages and limitations of quantum machine learning approaches.\\
