...
from qiskit.primitives import Estimator  # Aer primitive, not StatevectorEstimator
...
noise_model.add_all_qubit_quantum_error(
    thermal_relaxation_error(t1=50e3, t2=50e3, time=100, excited_state_population=0.),
    ['u2']
)

# Set up AerSimulator with noise
sim = AerSimulator(noise_model=noise_model, shots=1024)

# Use noisy AerSimulator in Estimator primitive
...
plt.savefig('tutorial1/loss_convergence_evolution.png')

# Save model
torch.save(model.state_dict(), "model_evolution.pt")

# Load and evaluate
qnn_test = create_qnn()
model_test = HybridNet(qnn_test)
model_test.load_state_dict(torch.load("model_evolution.pt"))
model_test.eval()
...
plt.grid(True)
plt.savefig("tutorial1/training_loss_per_epoch.png")
plt.show()
