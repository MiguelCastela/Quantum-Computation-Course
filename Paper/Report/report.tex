...
created with the best features of each of the tutorials and some evolutionary-inspired approaches to the hybrid quantum convolutional neural network (HQCNN)
...
\end{verbatim}
    ...
    \item The dataset is filtered to only have digits labeled 0 and 1 (first \texttt{n\_samples} of each)
        Then \texttt{np.append} function is used to concatenate the indices of the two classes (0 and 1) and then filter the dataset using these indices. The \texttt{np.where} function returns the indices of elements in an array that satisfy a given condition.
...
\end{verbatim}}
        ...
        Adam is a first-order gradient-based optimizer that combines momentum, which uses past gradients, and adaptive learning rates that scale the step size based on how frequently a parameter is updated. It is commonly used for deep learning tasks due to its speed and robustness. One of its key advantages is the adaptive learning rate per parameter, which often leads to good convergence. However, one downside of Adam is that it can sometimes generalize worse than optimizers such as Stochastic Gradient Descent (SGD). Other optimizers are explored in the second tutorial.\\
...
\section*{\fontsize{16}{20}\selectfont Diferent types of HCQNNs}
...
The NEQR and FRQI methods were tested in the HCQNN, instead of using ZZFeatureMap or ZfeatureMap.
...
\verb|rotation_blocks='ry',entanglement_blocks='cz',|
\verb|reps=1)|
...
\noindent
nn.sequential was used to create the output layer, which is a simple way to stack layers in PyTorch. The output layer consists of a linear layer followed by a softmax activation function. The linear layer maps the output of the quantum neural network to the desired number of classes (in this case, 2 for binary classification). Then, the softmax activation function converts the raw output scores into probabilities, making it suitable for multi-class classification tasks.\\
...
\subsection*{\fontsize{12}{16}\selectfont Tests with the first dataset}
...
\noindent
When the new IBM Cloud plataform became available,  a mini version of the HCQNN, with only 20 training samples and 10 testing samples and without any of the evolutionary operators (in the service of simplicity) was tested. Even with the new EstimatorV2 and these changes, the jobs didn't run to completion. By analising the jobs that were completed its possible to infer that the model was, on the last jobs, likely correctly infering the classes of the images (handwritten digits), this was inferred by the
\texttt{ev} $\approx 0.99 \pm 0.014$
result obtained in the primitive results from the last job. This indicates the QNN model confidently assigned a class close to 1, with a tight standard error (~1.4\%). As this isn't enough to infer global model performance, in future work this could possibly be achieved by adding a rule in the code that includes the true label of the image in the metadata. This could then be retrieved easily form the completed jobs. More minutes would also resolve the root problem.
